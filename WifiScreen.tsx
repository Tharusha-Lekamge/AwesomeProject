  ...
  return (
    <ScrollView>
      ...
      <View style={styles.wrapper}>
        ...
        <View style={{width: '100%'}}>
          <Pressable
            ...
            onPress={async () => {
              try {
                const wifiNetworks = await TetheringManager.getWifiNetworks(
                  true,
                );
            ...
            onPress={async () => {
              try {
                const maxSuggestions =
                  await TetheringManager.getMaxNumberOfNetworkSuggestions();
                ToastAndroid.show(
                  `${maxSuggestions} Max suggestions per app`,
                  ToastAndroid.SHORT,
                );
              } catch (error: any) {
                if (error instanceof TetheringError) {
          ...
          <Pressable
            ...
            onPress={async () => {
              try {
                const status =
                  await TetheringManager.isDeviceAlreadyConnected();
                ToastAndroid.show(
                  `Current WiFi status: ${status}`,
                  ToastAndroid.SHORT,
                );
              } catch (error: any) {
                if (error instanceof TetheringError) {
      ...
      <View style={{marginVertical: 15}}>
        {networks.map((n, i) => (
          <View style={styles.networksWrapper} key={i}>
